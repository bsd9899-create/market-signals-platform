...
def test_build_test_status_text_includes_all_required_fields() -> None:
    ...
    assert "✅ البوت يعمل بنجاح." in text
    assert "🟢 الحالة: يعمل" in text
    assert "🕒 الوقت:" in text
    assert "📊 السوق: OPEN" in text
...
def test_build_test_status_text_falls_back_to_closed_on_market_error() -> None:
    ...
    assert "📊 السوق: CLOSED" in text  # لا يفشل السطر بالكامل بسبب تعذّر حالة السوق
...
def test_handle_test_command_sends_two_messages_and_logs_success() -> None:
    ...
    _handle_test_command(_config(), logger, telegram_service, _FakeMarketService(), "999")

    assert len(telegram_service.sent_messages) == 2
    assert telegram_service.sent_messages[0][1].startswith("✅ البوت يعمل بنجاح.")
...
def test_handle_test_command_logs_failure_on_exception() -> None:
    ...
    logger = _FakeLogger()
    _handle_test_command(_config(), logger, _BrokenTelegramService(), _FakeMarketService(), "999")
